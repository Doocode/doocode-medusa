<script lang="ts">
	...
	let { images = [], index = $bindable(0), open = $bindable(false) }: Props = $props();
...
<svelte:window
	onmousemove={(e) => {
		...
		swipe.handleMove(e.clientX, e.clientY);
	}}
...
{#if open}
	<button
		transition:fade
		onclick={onClose}
		class="fixed inset-0 z-40 touch-none bg-black/70 backdrop-blur-xl"
	>
		<span class="sr-only">{m['slideshow.close']()}</span>
	</button>

	<Bindings isOpen={open} {onClose} {handlePrevious} {handleNext} {handleFirst} {handleLast} />

	{#if controller.uiVisible}
		<Header {onClose} />

		<Caption open caption={currentImage.alt} description={currentImage.description} />
	...
	<div transition:fade|global={{ duration: 150 }}>
		{#key index}
			<div
				class="pointer-events-none fixed inset-0 z-50 flex items-center justify-center overflow-hidden p-4 md:p-10"
				in:fly={{
			...
			>
				<div
					class="pointer-events-auto flex h-full w-full touch-none items-center justify-center transition-transform duration-75 ease-out select-none"
	...
	{#if controller.uiVisible}
		...
		<Footer {index} total={images.length} url={currentImage.src} />
	{/if}
{/if}
