	...
	{#if show}
		...
		>
			<Button
				size="icon"
				onclick={click}
				class="pointer-events-auto rounded-full hover:scale-120 active:scale-90 sm:size-16"
			>
				{#if shiftPressed}
...
<main class="pointer-events-none fixed inset-0 z-60 flex items-center justify-between md:px-8">
	<div>
		{@render navButton(
			hasPrevious,
			prevScale,
