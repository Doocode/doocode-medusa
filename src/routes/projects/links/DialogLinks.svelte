...
<Dialog.Root bind:open>
	<Dialog.Content
		class={cn(
			'transition-all duration-300 sm:max-w-115',
			viewMode === 'grid' && 'sm:max-w-3xl',
			accent && 'project_theme'
		)}
		style={accent ? `--accent-light: ${accent.light}; --accent-dark: ${accent.dark};` : undefined}
	>
		<Dialog.Header class="-mt-2 flex flex-row items-center justify-between space-y-0 pr-6">
			...
			<div class="flex items-center gap-2">
				...
				{/if}
				{@render toolButton(Search, showSearch, m['actions.search'](), toggleSearch)}
			</div>
		</Dialog.Header>

		{#if showSearch}
			<div class="-mx-2 pb-2 sm:mx-0">
				<SearchBar
					bind:value={searchQuery}
					placeholder={m['links.filter_placeholder']()}
					autofocus
				/>
			</div>
		{/if}
		...
		<div
			class={cn(
				'-mr-4 -ml-2 max-h-[60vh] overflow-y-auto py-2 pr-2 sm:-mr-2 sm:ml-0',
				viewMode === 'grid' ? 'grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4' : 'grid gap-2'
			)}
		>
			...
			{#if filteredLinks.length === 0}
				<div
					class="col-span-full flex flex-col items-center justify-center rounded-xl border-2 border-dashed bg-muted/30 py-8 text-muted-foreground"
				>
					<Unlink class="mb-2 size-15" />
