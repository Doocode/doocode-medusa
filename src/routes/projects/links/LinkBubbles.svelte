...
<div class={cn("flex isolate", s.container, className)}>
    {#each visibleLinks as link, i}
        <div class={cn("relative bg-primary text-primary-foreground rounded-full border-2 border-background flex items-center justify-center shadow-sm", s.bubble)} style="z-index: {10 - i}">
            <link.icon class={s.icon} />
        </div>
    {/each}
    {#if remainingCount > 0}
        <div class={cn("relative bg-muted text-muted-foreground rounded-full border-2 border-background flex items-center justify-center font-bold shadow-sm", s.bubble, s.text)} style="z-index: 0">
            +{remainingCount}
        </div>
    {/if}
</div>