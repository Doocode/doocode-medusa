...
>
	<span
		class={{
			'flex items-center gap-1 rounded-md px-2.5 py-1': true,
			...
			'bg-orange-300 text-orange-800 dark:bg-orange-800/70 dark:text-orange-300':
				type === 'bugfixes',
			'bg-amber-300 text-amber-800 dark:bg-amber-800/70 dark:text-amber-300':
				type === 'breaking_changes',
			'bg-purple-300 text-purple-800 dark:bg-purple-800/70 dark:text-purple-300':
				type === 'screenshots',
	...
	>
		...
		{:else if type === 'features'}
			<SquarePlus class="h-4.5 w-4.5" />
		{:else if type === 'improvements'}
			<Sparkles class="h-4.5 w-4.5" />
		{:else if type === 'bugfixes'}
			<Bug class="h-4.5 w-4.5" />
		{:else if type === 'screenshots'}
			<ImagePlay class="h-4.5 w-4.5" />
		{:else if type === 'removed'}
			<Trash2 class="h-4.5 w-4.5" />
		{:else if type === 'links'}
			<Link class="h-4.5 w-4.5" />
		{:else}
			<CircleDashed class="h-4.5 w-4.5" />
		{/if}
