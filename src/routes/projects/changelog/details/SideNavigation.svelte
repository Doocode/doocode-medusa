<script lang="ts">
	...
	const colors: Record<
		string,
		{ text: string; hoverBg: string; hoverText: string; ring: string; badge: string }
	> = {
		links: {
...
<nav class="-ml-4 grid">
	<h2 class="mx-4 mt-16 mb-4 text-2xl font-bold sm:mt-0 lg:mt-16">Navigation</h2>
	...
	{#snippet link(id: string, label: string, icon: Component, count: number)}
		...
		<a
			href="#{id}"
			class="group flex w-full items-center gap-3 rounded-lg px-4 py-3 transition-all duration-150
                {style.text} {style.hoverBg} {style.hoverText}
                focus:ring-2 focus:outline-none {style.ring}"
		>
			{#key icon}
				{@const Icon = icon}
			...
			<span
				class="-my-1 ml-auto flex aspect-square h-7 w-7 items-center justify-center rounded-full text-center text-sm font-bold {style.badge}"
			>
				{count}
			</span>
		</a>
	...
	{#if release.breakingChanges?.length}
		{@render link(
			'breaking_changes',
			m['changelog.breaking_changes.title'](),
			TriangleAlert,
	...
	{#if release.improvements?.length}
		{@render link(
			'improvements',
			m['changelog.improvements.title'](),
			Sparkles,
